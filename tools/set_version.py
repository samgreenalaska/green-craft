...
def main():
    ...
    # What GreenCraftSetup.exe reads. Same two facts as launcher.payload above, in a
    # format twenty lines of C can parse without a JSON reader -- the installer used
    # to walk the manifest, and that parser was the only part of it with real bugs.
    # Written here, from the same values, so the two cannot drift; verify_release.py
    # refuses to publish if they ever do.
    p = launcher["payload"]
    (REPO / "bootstrap.txt").write_text(
        "# GreenCraft installer pointer. Generated by tools/set_version.py -- do not edit.\n"
        f"version={version}\n"
        f"sha512={p['hashes']['sha512']}\n"
        f"url={p['downloads'][0]}\n",
        encoding="utf-8")
    print("wrote bootstrap.txt")
    print(f"\n{old} -> {version}")
